...
- **Host organization** — group the sidebar by source file or by tag (tag chips on rows, toggleable), search with `#tag` / `@user` prefixes, drag-to-reorder within a file, move hosts between config files (byte-identical block moves), duplicate as a template, rename the `Host` line itself, and give source files custom display names.
